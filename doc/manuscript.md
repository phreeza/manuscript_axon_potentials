...
distributions in axonal projection zones in the cortex and lateral geniculate
nucleus, and attributed these to axons by means of conduction velocity
estimation [@Mitzdorf1985Current]. In the results section we will present a
further new example of a dipolar, axonal field potential from the auditory
brainstem of the barn owl.

This discrepancy between the quadrupolar structure of EFPs generated by idealized axons,
as predicted by theory, and its dipolar nature, as observed in experiments, raises
the question of how axons are able to generate dipolar field potentials. The
aim of this study is to show how dipolar far fields in the EFP of axons can be
explained by the axons' anatomical structure. In particular, we show that the
branchings and terminations of axons in their projection area lead to a dipolar
EFP structure. Such axon bundles, often also called nerves or fascicles, exist
throughout the peripheral and central nervous system of most animals
[@kandel2000principles; @Hentschel1999Models; @Nornes1972Temporal;
...
are therefor related to the points of maximum curvature in the action potential
...
fiber), and the currents sum up across the fibers. This leads the second and
...
occurring in-between the bifurcation and termination zones. A notable point here is that the
...
frequency components are sometimes referred to as the LFP and the MUA.
...
the projection zone of the axon bundle, amplitudes showed the double-lobed
...
polarity in the middle of the projection zone of the bundle ([@fig:bigtree]B).
...
its projection zone, the number of bifurcations increases. The increase of
...
the end of the projection zone, the terminations outweigh the bifurcations. As
the axon bundle ends, there are no longer any bifurcations or terminations, and
the density decays toward zero. Overall, the amplitude of the maximum response
...
response, which reflects the high-frequency (>1 kHz) components, and is thought
to reflect spiking activity in extracellular recordings. The MUA did not show a
double-lobe, but rather appeared like an ellipsoid centered at the projection
zone ([@fig:bigtree]D).
...
radial fanning out of the bundle in the projection zone used in the simulation
...
simplified model also allowed us to consider a continuous fiber density instead
...
quadratic scaling with distance for distances larger than the extent of the
...
also quadratic for distances $\gtrsim 1$ mm, but the amplitudes are similar.
...
which is larger in the bifurcating case. To conclude, the projection zone
...
([@fig:distscaling]F). Here the moments are similar for very low frequencies,
then diverge at intermediate frequencies, before approaching each other again
for high frequencies. The quadrupole moment of the bifurcating case has a
...
projection zone has a supralinear contribution (factor greater than 10) for the
...
dorsally. The axon bundles reach the nucleus from the previous nucleus without
bifurcating, then bifrucate multiple times at the border of the NL, and then
terminate within NL. They have a strong directional preference and run roughly
in parallel. This well studied physiology and anatomy makes the system an ideal
...
The inferred fiber density is shown in [@fig:barnowl]D. The density is shown as
a function of dorsoventral depth, meaning that the y-axis of [@fig:barnowl]A
and [@fig:barnowl]B corresponds to the x-axis in [@fig:barnowl]D. The density
has its maximum at the center of the electrode array, and decays steadily to
both sides. This fiber density profile is consistent with the known anatomy of
axons in NL. The density does not decay to zero on either side, which may be
due to the limited range of recording locations. The fiber density beyond the
border of the nucleus in dorsal direction should be zero, because it is known
from anatomy that all fibers terminate within the nucleus.
...
components are governed by the local fiber density ([@fig:bigtree]).
...
When performing a typical current source density analysis, the local fiber
density cannot be untangled from the membrane current density
[@Nicholson1973Theoretical], and the membrane voltage, and therefor the
...
twice, based on different physiological substrates. Such a convergence might
point towards an evolutionary pressure favouring such an EFP structure in
...
structures and thus also strong, dipolar EFPs due to axons will be more prevalent in
...
the stimulus and to simulate a fiber tract prior to the projection zone, a
...
resulting response with a cutoff of 500 Hz.
