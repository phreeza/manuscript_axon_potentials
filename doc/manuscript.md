...
the EFP of thalamocortical afferents showed a polarity reversal as associated
...
lateral geniculate nucleus, and attributed these to axons by means of
conduction velocity estimation [@Mitzdorf1985Current]. Here we present
a further example of a strong dipolar, axonal field potential from the
...
(which would also require to include a synchronization of the spike times to
the auditory stimulation frequency, called phase locking), and this pulse of
...
The two frequency bands showed a qualitatively different spatiotemporal
...
remained. The distribution of maximum amplitudes of these responses are shown
...
In [@fig:bigtree]B, the LFP responses mostly showed monophasic deflections that
were elicited by the population firing rate pulse, in a manner similar to that
observed in [@fig:simpletree]E. Such deflections were visible at all recording
locations. In the radial direction away from the axon tree, i.e. in horizontal
...
accordance to the local number of fibers (teal-colored histogram). The local number of
...
anatomy (i.e. the presence of a projection zone), and temporal frequency range
...
activity as a temporal frequency of 1\ kHz and propagates at 1\ m/s, the
...
when compared to the control case greater than 10 is supralinear. In
...
based on a more detailed skull model and paired recordings of ABR and EFP will
hopefully improve our understanding of the true contributions axons make to the
ABR.
