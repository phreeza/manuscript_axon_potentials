...
200 $\mu$m, leading to a total number of 8 fibers leaving the bifurcation zone
...
a constant background rate (0.1 spikes/ms) and a Gaussian pulse of heightened
activity (2 spikes/ms). The standard deviation of the pulse was 1 ms, resulting
...
understand whether and how axons can generate a dipolar EFP, we turned to
...
150\ $\mu$m from the axons. The vertical depth is indicated by the plot
position, spaced by 200\ $\mu$m. (**A**) Action potential in an infinitely
...
terminations. Note the reduced voltage scales in C and D, which correspond to
the different number of fibers, (**E**) Response in a population of 100
randomized morphologies, three of which are shown schematically. Activity
consists of spontaneous background activity (0.1 spikes/ms) superimposed with a pulse of
heightened spike rate (2.9 spikes/ms). Spike rate and example spike times for the three
morphologies are shown at the top. Right: gray lines show activity averaged over 40
trials, while the black lines show the analytically determined limit of
infinitely many trials. Note the extended time scale different from A-D.
...
EFP, we low-pass filtered the EFP responses with a cut-off frequency of 1\ kHz
...
direction along the axon tree, i\.e\. in vertical direction in the figure, the
...
the number of branchings and terminations per 200 $\mu$m bin (purple histogram in
[@fig:bigtree]C).

Along the nerve trunk the number of fibers is constant. As the axon bundle
reaches its terminal zone, the number of bifurcations increases (purple bars point
...
locations of the colored dots in *A*, also show a polarity reversal. The
...
distance of 100\ $\mu$m from the trunk (indicated by arrow in A). It closely
...
spiking activity. In our example in [@fig:bigtree], the maximum amplitudes of
the MUA did not show the double-lobe shape observed for the LFP in
[@fig:bigtree]A. Rather, the maximum amplitudes appeared like an ellipsoid
centered at the terminal zone ([@fig:bigtree]D).
...
The dependence of the MUA amplitude on the axial location is further examined
...
of an axon terminal structure on the spatial reach of the EFP. Motivated by the
...
bundle. As in [@fig:bigtree], we simulated an axon bundle containing a terminal zone with
...
Furthermore, the simplified model discarded the detailed conductance-based
...
possible to calculate the membrane currents necessary for determination of the
...
short distances $< 100$\ $\mu$m. At these short distances, the difference is
...
axial distance 1\ $\mu$m was then proportional to the dipole moments.
...
[@wagner09]. The frequency of the high-frequency ([@fig:expmethod]B) ringing
...
([@fig:expmethod]C). We filtered the data to roughly separate these components.
...
of a 40µm thick transverse Nissl stained section through the dorsal brain stem,
...
(\*), and penetrated into the nucleus laminaris (NL) Scale bar, 1mm. The
...
magenta rectangle. The low-frequency component (**B**) and high-frequency
component (**C**) are ordered the same as the electrodes, with three examples
connected to their recording sites by black lines. Time scale in B and C is
identical as indicated by the scale bar, and voltage scale as indicated by
...
of the recording array, at around 850\ $\mu$m penetration depth. The axonal
...
@McColgan2014Functional, with a upper confidence bound of 6 m/s. Toward the
edges (< 100 and > 1400\ $\mu$m), the amplitude decays. In the central region
(400-1200\ $\mu$m, a systematic shift in delay can be observed, while the
...
Gaussian distribution with mean zero and a width of 300 $\mu$m.
