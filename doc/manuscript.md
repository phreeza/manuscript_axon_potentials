...
abstract: Extracellular field potentials (EFPs) are an important source of information in neuroscience, but their physiological basis is in many cases still a matter of debate. Axonal sources are typically discounted in modeling and data analysis. Here, we experimentally and theoretically quantified the contributions of axons to EFPs. We first used a detailed biophysical model and a simplified analytical model of action potentials propagating along axons. Predicted EFPs were especially prominent when there was a terminal zone in which axons branch and terminate in close succession, as found in many brain regions. Our models predicted a dipolar far field and polarity reversal at the center of the terminal zone. We then confirmed these predictions using data obtained from the barn owl auditory brainstem. EFPs were recorded at different locations in nucleus laminaris using a multielectrode array. These results demonstrate that axonal terminal zones produce EFPs with considerable amplitude and spatial reach.
...
...
the EFP of axonal responses can also have a dipolar structure. For example,
...
a further well developed example of a dipolar, axonal field potential from the
...
and width of 300\ µm for $z>0$\ µm. The second case was that of an axon bundle
...
in the control condition, too. However, comparing the control to the
