...
distributions in axonal terminal fields in the cortex and lateral geniculate
...
that the branchings and terminations of axons in their terminal field area lead
...
the center of the terminal field largely cancelled out because the differences
...
bifurcations and then reaches a terminal field. Within this projection zone,
...
the terminal field of the axon bundle, amplitudes showed the double-lobed
...
polarity in the middle of the terminal field of the bundle ([@fig:bigtree]B).
...
terminal field. This means that the absolute value of the response amplitude
...
its terminal field, the number of bifurcations increases. The increase of
bifurcations is followed by an increase of terminations. In the middle of the
terminal field, the number of bifurcations and terminations are equal. At
the same depth, the amplitude of the low-frequency component crosses zero.  At
the end of the terminal field, the terminations outweigh the bifurcations. As
...
double-lobe, but rather appeared like an ellipsoid centered at the terminal field
([@fig:bigtree]D).
...
axon bundle, one containing a terminal field with bifurcations, and one
...
radial fanning out of the bundle in the terminal field used in the simulation
...
which is larger in the bifurcating case. To conclude, the terminal field
...
In [@fig:distscaling]G we compare the relative impact of the terminal field on
...
contribution from the terminal field. The dipole moment approaches a factor of
10, meaning that the terminal field contributes. In both components, the
...
terminal field has a supralinear contribution (factor greater than 10) for the
dipole at frequencies below 3.5 kHz and for the quadrupolar component at
frequencies between 1 and 4.5 kHz.

Together, these observations show us that the terminal field makes a strong
...
distances are calculated from the center of the terminal field. For
...
fibers cannot be untangled from the membrane current density
...
the number of fibers is variable in space, in particular in the terminal field,
...
dipolar structure in terminal fields. Our modelling approach provides a novel
...
the stimulus and to simulate a fiber tract prior to the terminal field, a
sequence of 10 active and passive segments without bifurcations was assumed
before the terminal field. To this root, segments were appended iteratively.
...
the end of the terminal field. The distribution of bifurcations had it's
