...
stimuli were generated as a single half-wave of a 5 kHz sine tone. Miniature
...
alternation of active and passive segment in each resulting branch.
...
initial segment was not modeled explicitly.

- conductivity details
- concrete numbers for segment lenghts
- concrete numbers for logistic functions
...
- bebin with motivation
- cable theiry reference

\label{sec:efpresp}
We define the spatial dimension in cylindrical coordinates
$\mathbf{r}=(\rho,\varphi,z)$, and we consider a simple model axon bundle that
...
coordinate. From linear cable theory, we can then obtain the following expression for
