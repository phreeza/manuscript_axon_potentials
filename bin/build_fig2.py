...
def calc_mua(x, dt=0.0025, f_hp=2000., f_lp=500.):
    sos = signal.butter(20, 2 * f_hp * dt / 1000., btype='highpass', output='sos')
    raw = signal.sosfiltfilt(sos, x, axis=1)
    ret = raw.copy()
    ret[ret < 0.] = 0.
...
def run_fig2(pot, n_rows, n_cols, **params):
    ...
    for freq_n in range(2):
        if freq_n == 0:
            sos = signal.butter(20, 2 * filt_freq * dt / 1000., btype=filter_type[freq_n], output='sos')
            fted = signal.sosfiltfilt(sos, raw_pot, axis=1)
        else:
